...
module.exports = exports = function(config, socket, maxRelayLimitPerUser) {
    ...
    socket.on('join-broadcast', function(user) {
        try {
            if (!users[user.userid]) {
                ...
                notifyViewersAboutNumbersOfViewers(user.broadcastId);
            }
    ...
    socket.on('send-broadcast-message', function(broadcastId, callback) {
    ...
    function getNumberOfViewers(broadcastId) {
        try {
            var numberOfUsers = 0;
            Object.keys(users).forEach(function(uid) {
                var user = users[uid];
    ...
    function notifyViewersAboutNumbersOfViewers(broadcastId, userLeft) {
        try {
            if (!broadcastId || !users[broadcastId] || !users[broadcastId].socket) return;
            let viewers = getNumberOfBroadcastViewers(broadcastId);

            if (userLeft) {
                viewers = viewers.filter(viewerId => viewerId !== userLeft);
            }

            Object.keys(users).forEach(function(uid) {
                const user = users[uid];
                if (user.broadcastId === broadcastId) {
                    users[uid].socket.emit('number-of-broadcast-viewers-updated', {
                        viewers: viewers,
    ...
    socket.ondisconnect = function() {
        try {
            ...
            if (user.isBroadcastInitiator === false) {
                ...
                notifyViewersAboutNumbersOfViewers(user.broadcastId, user.userid);
            }
