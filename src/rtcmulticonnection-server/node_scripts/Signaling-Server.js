...
const conferenceService = require("../../../src/services/conference.service.js");
...
module.exports = exports = function (socket, config) {
  ...
  function onConnection(socket) {
    ...
    // testing signals
    socket.on("mute video", function (roomid) {
      listOfRooms[roomid].participants.forEach((pid) => {
        if (pid === socket.userid) return;

        listOfUsers[pid].socket.emit("mute video", socket.userid);
      });
    });

    socket.on("unmute video", function (roomid) {
      listOfRooms[roomid].participants.forEach((pid) => {
        if (pid === socket.userid) return;

        listOfUsers[pid].socket.emit("unmute video", socket.userid);
      });
    });
    ...
    function joinARoom(message) {
      try {
        ...
        if (
          listOfRooms[roomid].participants.length >= listOfRooms[roomid].maxParticipantsAllowed &&
        ...
        if (
          ...
          (listOfRooms[roomid].session.oneway === true || listOfRooms[roomid].session.broadcast === true)
        ) {
    ...
    function closeOrShiftRoom() {
      try {
        ...
        //     conferenceService.closeConference(currentTime, socket.userid).catch(error => console.log(error));
        // }
      } catch (e) {
    ...
    socket.on("open-room", function (arg, callback) {
      ...
      try {
        callback(true);
        // const isPrivate = (arg.identifier && arg.identifier.toString().length) ? false : true;
        // const confName = listOfUsers[socket.userid].extra.confName || 'Видеотрансляция';

        // conferenceService.openConference(socket.userid, confName, isPrivate, socket.userid, 'caht_' + socket.userid)
        //     .then(() => callback(true))
        //     .catch(() =>  callback(false, CONST_STRINGS.ROOM_NOT_AVAILABLE));
      } catch (e) {
        pushLogs(config, "open-room", e);
      }
    });
