...
var RTCMultiConnection = function (roomid, forceOptions) {
  function SocketConnection(connection, connectCallback) {
    ...
    function updateExtraBackup(remoteUserId, extra) {
      connection.peersBackup[remoteUserId] || (connection.peersBackup[remoteUserId] = { userid: remoteUserId, extra: {} }),
      ...
      if (message.remoteUserId == connection.userid) {
        if (
          ...
          if ("ended" === action || "inactive" === action || "stream-removed" === action)
            return (
              connection.peersBackup[stream.userid] && (stream.extra = connection.peersBackup[stream.userid].extra),
        ...
        if (message.message.allParticipants)
          return (
            message.message.allParticipants.indexOf(message.sender) === -1 && message.message.allParticipants.push(message.sender),
        ...
        if (
          ...
          (connection.attachStreams.length && (connection.waitingForLocalMedia = !1), connection.waitingForLocalMedia)
        )
    ...
    (connection.peers = {
        ...
        return (
          userPreferences || (userPreferences = {}),
          {
            ...
            onDataChannelMessage: function (message) {
              if (
                (!connection.fbr && connection.enableFileSharing && initFileBufferReader(),
        ...
        if (!(connection.maxParticipantsAllowed <= connection.getAllParticipants().length)) {
          if (
            ...
            connection.isInitiator &&
              ...
              ((userPreferences.isOneWay = !1), (userPreferences.isDataOnly = !1), (userPreferences.session = connection.session)),
            !userPreferences.isOneWay && !userPreferences.isDataOnly)
          )
            return (
              (userPreferences.isOneWay = !0),
              void this.onNegotiationNeeded({ enableMedia: !0, userPreferences: userPreferences }, remoteUserId)
            );
        ...
        return peer.getSenders && "function" == typeof peer.getSenders && peer.getSenders().length
          ? void peer.getSenders().forEach(function (rtpSender) {
              isVideoTrack &&
                ...
                ((connection.peers[remoteUserId].peer.lastVideoTrack = rtpSender.track), rtpSender.replaceTrack(track)),
                isVideoTrack ||
                  "audio" !== rtpSender.track.kind ||
                  ((connection.peers[remoteUserId].peer.lastAudioTrack = rtpSender.track), rtpSender.replaceTrack(track));
            })
      ...
      (this.addNegotiatedMessage = function (message, remoteUserId) {
        if (message.type && message.sdp)
          return (
            "answer" == message.type && connection.peers[remoteUserId] && connection.peers[remoteUserId].addRemoteSdp(message),
            "offer" == message.type &&
        ...
        if (message.candidate)
          return (
            connection.peers[remoteUserId] && connection.peers[remoteUserId].addRemoteCandidate(message),
        ...
        if (message.enableMedia) {
          ...
          connection.attachStreams.forEach(function (stream) {
            ...
            self.onNegotiationNeeded({ readyForOffer: !0, userPreferences: message.userPreferences }, remoteUserId);
        }
  ...
  function getRMCMediaElement(stream, callback, connection) {
    ...
    var isAudioOnly = !1;
    getTracks(stream, "video").length || stream.isVideo || stream.isScreen || (isAudioOnly = !0),
      ...
      (("Firefox" === DetectRTC.browser.name && audioPlusTab !== !1) ||
        (!("Chrome" !== DetectRTC.browser.name || DetectRTC.browser.version < 50) &&
          (typeof audioPlusTab === !0 ||
            (!("undefined" != typeof audioPlusTab || !connection.session.audio || !connection.session.screen || connection.session.video) &&
              ((audioPlusTab = !0), !0)))))
    );
  }
  ...
  function PeerInitiator(config) {
    ...
    function createOfferOrAnswer(_method) {
      peer[_method](defaults.sdpConstraints).then(
        function (localSdp) {
          "Safari" !== DetectRTC.browser.name && (localSdp.sdp = connection.processSdp(localSdp.sdp)),
    ...
    if (
      ...
      try {
        ...
        "undefined" != typeof connection.iceCandidatePoolSize && (params.iceCandidatePoolSize = connection.iceCandidatePoolSize),
          "undefined" != typeof connection.bundlePolicy && (params.bundlePolicy = connection.bundlePolicy),
          "undefined" != typeof connection.rtcpMuxPolicy && (params.rtcpMuxPolicy = connection.rtcpMuxPolicy),
          connection.sdpSemantics && (params.sdpSemantics = connection.sdpSemantics || "unified-plan"),
          (connection.iceServers && connection.iceServers.length) || ((params = null), (connection.optionalArgument = null)),
    ...
    !peer.getRemoteStreams &&
      ...
      localStreams.forEach(function (localStream) {
        (config.remoteSdp && config.remoteSdp.remotePeerSdpConstraints && config.remoteSdp.remotePeerSdpConstraints.dontGetRemoteStream) ||
    ...
    (peer.ontrack = function (event) {
      ...
      (this.addRemoteSdp = function (remoteSdp, cb) {
        (cb = cb || function () {}),
          "Safari" !== DetectRTC.browser.name && (remoteSdp.sdp = connection.processSdp(remoteSdp.sdp)),
          peer
    ...
    config.remoteSdp && (isOfferer = !1),
      ...
      config.remoteSdp &&
        (config.remoteSdp.remotePeerSdpConstraints && (sdpConstraints = config.remoteSdp.remotePeerSdpConstraints),
        (defaults.sdpConstraints = setSdpConstraints(sdpConstraints)),
        this.addRemoteSdp(config.remoteSdp, function () {
          createOfferOrAnswer("createAnswer");
        })),
      ("two-way" != connection.session.audio && "two-way" != connection.session.video && "two-way" != connection.session.screen) ||
        (defaults.sdpConstraints = setSdpConstraints({
          OfferToReceiveAudio:
            "two-way" == connection.session.audio ||
            (config.remoteSdp &&
  ...
  !(function (that) {
    that &&
      ...
      "undefined" == typeof location && (that.location = { protocol: "file:", href: "", hash: "", origin: "self" }),
    ...
    (function () {
      function getBrowserInfo() {
        ...
        if ((isSafari && !isChrome && nAgt.indexOf("CriOS") !== -1 && ((isSafari = !1), (isChrome = !0)), isOpera)) {
          ...
          isIE
            ...
            ? ((verOffset = nAgt.indexOf("Safari")),
              ...
              (verOffset = nAgt.indexOf("Version")) !== -1 && (fullVersion = nAgt.substring(verOffset + 8)),
              navigator.userAgent.indexOf("Version/") !== -1 && (fullVersion = navigator.userAgent.split("Version/")[1].split(" ")[0]))
            : isFirefox
        ...
        return (
          isEdge && ((browserName = "Edge"), (fullVersion = navigator.userAgent.split("Edge/")[1])),
      ...
      function getIPs(callback, stream) {
        ...
        if ("undefined" != typeof document && "function" == typeof document.getElementById) {
          ...
          if (RTCPeerConnection) {
            ...
            if (
              (stream &&
                (pc.addStream ? pc.addStream(stream) : pc.addTrack && stream.getTracks()[0] && pc.addTrack(stream.getTracks()[0], stream)),
        ...
        navigator.enumerateDevices(function (devices) {
          devices.forEach(function (_device) {
            ...
            alreadyUsedDevices[device.deviceId + device.label + device.kind] ||
              ...
              device.label
                ? ("videoinput" !== device.kind || isWebsiteHasWebcamPermissions || (isWebsiteHasWebcamPermissions = !0),
                  "audioinput" !== device.kind || isWebsiteHasMicrophonePermissions || (isWebsiteHasMicrophonePermissions = !0))
              ...
              "audioinput" === device.kind &&
                ((hasMicrophone = !0), audioInputDevices.indexOf(device) === -1 && audioInputDevices.push(device)),
              "audiooutput" === device.kind &&
                ((hasSpeakers = !0), audioOutputDevices.indexOf(device) === -1 && audioOutputDevices.push(device)),
              "videoinput" === device.kind &&
                ((hasWebcam = !0), videoInputDevices.indexOf(device) === -1 && videoInputDevices.push(device)),
      ...
      if (isNodejs) {
        ...
        browserFakeUserAgent =
          "Nodejs/" + version + " (NodeOS) AppleWebKit/" + version + " (KHTML, like Gecko) Nodejs/" + version + " Nodejs/" + version;
      }
      !(function (that) {
        "undefined" == typeof window &&
          ("undefined" == typeof window && "undefined" != typeof global
            ? ((global.navigator = {
              ...
              (that.window = global))
            : "undefined" == typeof window,
          "undefined" == typeof location && (that.location = { protocol: "file:", href: "", hash: "" }),
          "undefined" == typeof screen && (that.screen = { width: 0, height: 0 }));
      })("undefined" != typeof global ? global : window);
      var navigator = window.navigator;
      "undefined" != typeof navigator
        ? ("undefined" != typeof navigator.webkitGetUserMedia && (navigator.getUserMedia = navigator.webkitGetUserMedia),
          "undefined" != typeof navigator.mozGetUserMedia && (navigator.getUserMedia = navigator.mozGetUserMedia))
      ...
      if (
        (DetectRTC.browser.isChrome && DetectRTC.browser.version >= 35
          ...
          ? (isScreenCapturingSupported = !0)
          : "Android" === DetectRTC.osName && DetectRTC.browser.isChrome && (isScreenCapturingSupported = !0),
        !/^(https:|chrome-extension:)$/g.test(location.protocol || ""))
      ) {
        var isNonLocalHost =
          "undefined" != typeof document &&
          ...
          document.domain.search &&
          document.domain.search(/localhost|127.0./g) === -1;
        isNonLocalHost && (DetectRTC.browser.isChrome || DetectRTC.browser.isEdge || DetectRTC.browser.isOpera)
      ...
      (DetectRTC.displayResolution = displayResolution),
        ...
        "undefined" != typeof MediaDevices ? (DetectRTC.MediaDevices = MediaDevices) : (DetectRTC.MediaDevices = []),
        (DetectRTC.hasMicrophone = hasMicrophone),
        (DetectRTC.hasSpeakers = hasSpeakers),
        (DetectRTC.hasWebcam = hasWebcam),
        (DetectRTC.isWebsiteHasWebcamPermissions = isWebsiteHasWebcamPermissions),
    ...
    CodecsHandler = (function () {
      function preferCodec(sdp, codecName) {
        ...
        return info.videoCodecNumbers
          ? "vp8" === codecName && info.vp8LineNumber === info.videoCodecNumbers[0]
            ? sdp
            : "vp9" === codecName && info.vp9LineNumber === info.videoCodecNumbers[0]
            ? sdp
            : "h264" === codecName && info.h264LineNumber === info.videoCodecNumbers[0]
            ? sdp
              ...
              line
                ...
                .forEach(function (codecNumber) {
                  (codecNumber = codecNumber.trim()),
                    codecNumber &&
                      codecNumber.length &&
                      (info.videoCodecNumbers.push(codecNumber), (info.videoCodecNumbersOriginal = line));
                })),
              line.indexOf("VP8/90000") === -1 || info.vp8LineNumber || (info.vp8LineNumber = line.replace("a=rtpmap:", "").split(" ")[0]),
              line.indexOf("VP9/90000") === -1 || info.vp9LineNumber || (info.vp9LineNumber = line.replace("a=rtpmap:", "").split(" ")[0]),
      ...
      function setOpusAttributes(sdp, params) {
        ...
        var appendOpusNext = "";
        return (
          (appendOpusNext += "; stereo=" + ("undefined" != typeof params.stereo ? params.stereo : "1")),
          ...
          "undefined" != typeof params.maxaveragebitrate &&
            (appendOpusNext += "; maxaveragebitrate=" + (params.maxaveragebitrate || 1048576)),
  ...
  var OnIceCandidateHandler = (function () {
      function processCandidates(connection, icePair) {
        ...
        if (
          (isNull(iceRestrictions.reflexive) || (stun = iceRestrictions.reflexive),
          ...
          if ((protocol.udp || !candidate.match(/ udp /g)) && (protocol.tcp || !candidate.match(/ tcp /g)))
            return (
              connection.enableLogs && console.debug("Your candidate pairs:", candidate),
              { candidate: candidate, sdpMid: icePair.sdpMid, sdpMLineIndex: icePair.sdpMLineIndex }
            );
        }
      }
    ...
    remove: function (idInstance) {
      ...
      if (stream) {
        ...
        this.queueRequests.indexOf(options) &&
          (delete this.queueRequests[this.queueRequests.indexOf(options)], (this.queueRequests = removeNullEntries(this.queueRequests))),
      ...
      function handle(connection) {
        ...
        (connection.onFileStart = function (file) {
          ...
          (connection.onFileProgress = function (chunk) {
            ...
            helper &&
              ((chunk.remoteUserId && !(helper = progressHelper[chunk.uuid][chunk.remoteUserId])) ||
  ...
  !(function (connection) {
    ...
    function joinRoom(connectionDescription, cb) {
      connection.socket.emit(
        ...
        function (isRoomJoined, error) {
          if (isRoomJoined === !0) {
            if (
              (connection.enableLogs && console.log("isRoomJoined: ", isRoomJoined, " roomid: ", connection.sessionid),
              connection.peers[connection.sessionid])
            )
              return;
            mPeer.onNegotiationNeeded(connectionDescription);
          }
          isRoomJoined === !1 && connection.enableLogs && console.warn("isRoomJoined: ", error, " roomid: ", connection.sessionid),
            cb(isRoomJoined, connection.sessionid, error);
        }
      );
    }
    function openRoom(callback) {
      connection.enableLogs && console.log("Sending open-room signal to socket.io"),
        ...
        connection.socket.emit(
          ...
          function (isRoomOpened, error) {
            isRoomOpened === !0 &&
              (connection.enableLogs && console.log("isRoomOpened: ", isRoomOpened, " roomid: ", connection.sessionid),
              callback(isRoomOpened, connection.sessionid)),
              isRoomOpened === !1 &&
                (connection.enableLogs && console.warn("isRoomOpened: ", error, " roomid: ", connection.sessionid),
    ...
    function beforeJoin(userPreferences, callback) {
      ...
      return session.oneway && "two-way" !== session.audio && "two-way" !== session.video && "two-way" !== session.screen
        ? void callback()
        : (session.oneway && session.audio && "two-way" === session.audio && (session = { audio: !0 }),
          void (
            (session.audio || session.video || session.screen) &&
            (session.screen
              ? "Edge" === DetectRTC.browser.name
                ? navigator.getDisplayMedia({ video: !0, audio: isAudioPlusTab(connection) }).then(
                    function (screen) {
                      (screen.isScreen = !0),
                ...
                : connection.invokeGetUserMedia(
                    { audio: isAudioPlusTab(connection), video: !0, isScreen: !0 },
                    (!session.audio && !session.video) || isAudioPlusTab(connection)
                      ? callback
                      : connection.invokeGetUserMedia(null, callback)
                  )
    ...
    (mPeer.onGettingLocalMedia = function (stream, callback) {
      ...
      (mPeer.onNegotiationNeeded = function (message, remoteUserId, callback) {
        ...
        message.remoteUserId && message.message && message.sender && (messageToDeliver = message),
      ...
      (connection.join = function (remoteUserId, options) {
        (connection.sessionid =
          (!!remoteUserId && (remoteUserId.sessionid || remoteUserId.remoteUserId || remoteUserId)) || connection.sessionid),
          (connection.sessionid += "");
        var localPeerSdpConstraints = !1,
          remotePeerSdpConstraints = !1,
          isOneWay = !1,
      ...
      window.ignoreBeforeUnload
        ...
        : ((connection.closeBeforeUnload = !0), window.addEventListener("beforeunload", connection.onbeforeunload, !1)),
      ...
      (connection.processSdp = function (sdp) {
        return isUnifiedPlanSupportedDefault()
          ...
          ? sdp
          : ("VP8" === connection.codecs.video.toUpperCase() && (sdp = CodecsHandler.preferCodec(sdp, "vp8")),
            "VP9" === connection.codecs.video.toUpperCase() && (sdp = CodecsHandler.preferCodec(sdp, "vp9")),
            "H264" === connection.codecs.video.toUpperCase() && (sdp = CodecsHandler.preferCodec(sdp, "h264")),
      ...
      (connection.mediaConstraints = {
        ...
        video: {
          ...
          optional: connection.bandwidth.video
            ? [{ bandwidth: 8 * connection.bandwidth.video * 1024 || 1048576 }, { facingMode: "user" }]
            : [{ facingMode: "user" }],
        },
      }),
      "Firefox" === DetectRTC.browser.name && (connection.mediaConstraints = { audio: !0, video: !0 }),
          ...
          if (
            ...
            if ("Firefox" === DetectRTC.browser.name)
              return void (connection.mediaConstraints.audio !== !0
                ...
                : (connection.mediaConstraints.audio = { deviceId: lastAudioDevice.id }));
            1 == connection.mediaConstraints.audio && (connection.mediaConstraints.audio = { mandatory: {}, optional: [] }),
          ...
          if (lastVideoDevice) {
            if ("Firefox" === DetectRTC.browser.name)
              return void (connection.mediaConstraints.video !== !0
                ...
                : (connection.mediaConstraints.video = { deviceId: lastVideoDevice.id }));
            1 == connection.mediaConstraints.video && (connection.mediaConstraints.video = { mandatory: {}, optional: [] }),
      ...
      (connection.addStream = function (session, remoteUserId) {
        ...
        return session.getTracks
          ? (connection.attachStreams.indexOf(session) === -1 &&
              (session.streamid || (session.streamid = session.id), connection.attachStreams.push(session)),
      ...
      (connection.applyConstraints = function (mediaConstraints, streamid) {
        ...
        if (streamid) {
          var stream;
          return (
            connection.streamEvents[streamid] && (stream = connection.streamEvents[streamid].stream),
      ...
      (connection.replaceTrack = function (session, remoteUserId, isVideoTrack) {
        ...
        if (session instanceof MediaStream)
          return (
            getTracks(session, "video").length && replaceTrack(getTracks(session, "video")[0], remoteUserId, !0),
            void (getTracks(session, "audio").length && replaceTrack(getTracks(session, "audio")[0], remoteUserId, !1))
          );
          ...
          "oninactive" in stream && (streamEndedEvent = "inactive"),
            stream.addEventListener(
              ...
              function () {
                ...
                (streamEvent.userid === connection.userid && "remote" === streamEvent.type) ||
                  (connection.peersBackup[streamEvent.userid] && (streamEvent.extra = connection.peersBackup[streamEvent.userid].extra),
                  connection.onstreamended(streamEvent),
                  delete connection.streamEvents[stream.streamid]);
              },
      ...
      (connection.getSocket = function (callback) {
        return (
          !callback && connection.enableLogs && console.warn("getSocket.callback paramter is required."),
          (callback = callback || function () {}),
          connection.socket
    ...
    if (
      ((connection.streamEvents = {
        ...
        selectAll: function (options) {
          options || (options = { local: !0, remote: !0, isScreen: !0, isAudio: !0, isVideo: !0 }),
            ...
            Object.keys(connection.streamEvents).forEach(function (key) {
              ...
              if (skipStreams.indexOf(key) === -1) {
                ...
                options.local && "local" === event.type && (ignore = !1),
                  ...
                  options.userid && event.userid === options.userid && (ignore = !1),
      ...
      (connection.checkPresence = function (roomid, callback) {
        return (
          ...
          "SSEConnection" === SocketConnection.name
            ? void SSEConnection.checkPresence(roomid, function (isRoomExist, _roomid, extra) {
            ...
            : connection.socket
            ? void connection.socket.emit("check-presence", roomid + "", function (isRoomExist, _roomid, extra) {
                connection.enableLogs && console.log("checkPresence.isRoomExist: ", isRoomExist, " roomid: ", _roomid),
                  callback(isRoomExist, _roomid, extra);
              })
            : void connection.connectSocket(function () {
      ...
      (connection.setUserPreferences = function (userPreferences) {
        return (
          connection.dontAttachStream && (userPreferences.dontAttachLocalStream = !0),
    ...
    (connection.getExtraData = function (remoteUserId, callback) {
      ...
      return "function" == typeof callback
        ? void connection.socket.emit("get-remote-user-extra-data", remoteUserId, function (extra, remoteUserId, error) {
      ...
      (connection.onUserIdAlreadyTaken = function (useridAlreadyTaken, yourNewUserId) {
        connection.close(),
          ...
          connection.enableLogs && console.warn("Userid already taken.", useridAlreadyTaken, "Your new userid:", connection.userid);
      }),
      ...
      (connection.setPassword = function (password, callback) {
        (callback = callback || function () {}),
          connection.socket
            ...
            : ((connection.password = password), callback(!0, connection.sessionid, null));
      }),
